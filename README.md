...
- 执行权锁在闸门上：只有已批准（approved）指令在设备空闲时才能开始执行，获取指令状态、闸门状态与获取锁在同一事务提交。
- 闸门同时只接受一条执行指令；占用冲突返回 409 `gate_occupied` 并携带对方指令编号，失败方保持原状态。锁行以闸门维度唯一索引，两条前后脚到达的请求只有一个能获取执行权。
- 指令完成、中止或执行回执失败都会在同一事务释放执行权；中止/失败同时按既有规则将闸门落为 `locked`，需经闸门自身流程解锁后才能再次作业。
...
- 闸门与指令查询均返回当前执行权（`executionLock`）：闸门页显示占用指令，指令页显示执行权与执行状态；获取/释放均落审计。
