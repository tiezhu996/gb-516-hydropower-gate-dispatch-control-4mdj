...
func New(cfg config.Config, db *gorm.DB, redisClient *redis.Client, logger *slog.Logger) *gin.Engine {
	...
	gateExecutionLockRepository := repository.NewGateExecutionLockRepository(db)
	reservoirService := service.NewReservoirService(reservoirRepository, securityService)
	gateUnitService := service.NewGateUnitService(gateUnitRepository, reservoirRepository, gateExecutionLockRepository, securityService)
	operationDirectiveService := service.NewOperationDirectiveService(operationDirectiveRepository, gateUnitRepository, gateExecutionLockRepository, securityService)
	executionConfirmationService := service.NewExecutionConfirmationService(executionConfirmationRepository, operationDirectiveRepository, gateUnitRepository, gateExecutionLockRepository, securityService)
